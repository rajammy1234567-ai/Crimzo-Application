...
async function getHostChatRate(hostId, settings) {
  const host = await User.findById(hostId).select(
    "chat_rate_per_min_inr voice_rate_per_min_inr",
  );
...
async function emitTalkPrivateReady(io, talkSession) {
  ...
  const talker = await User.findById(talkSession.talker_id)
    .select("username avatar")
    .lean();
  const host = await User.findById(talkSession.host_id)
    .select("username avatar")
    .lean();
  const payload = {
...
exports.verifyPrivateTalkAccess = async (talkSessionId, userId) => {
  ...
  if (String(talk.host_id) !== uid && String(talk.talker_id) !== uid)
    return null;
  return talk;
};
...
exports.requestTalk = async (req, res) => {
  try {
    ...
    if (!session || session.status !== "active") {
      return res
        .status(400)
        .json({ error: "This live stream is no longer active" });
    }
    ...
    if (String(hostId) === String(requesterId)) {
      return res
        .status(400)
        .json({ error: "Hosts cannot request talk on their own stream" });
    }

    const requester = await User.findById(requesterId).select(
      "wallet_balance username avatar",
    );
    const balance = requester?.wallet_balance || 0;
    const talkPayload = buildLiveTalkBalancePayload(
      balance,
      settings,
      chatRate,
    );
...
exports.respondTalk = async (req, res) => {
  try {
    ...
    const hostUser = await User.findById(hostId).select(
      "chat_rate_per_min_inr voice_rate_per_min_inr",
    );
    ...
    if (!requestId || !["accept", "reject"].includes(action)) {
      return res
        .status(400)
        .json({ error: "requestId and action (accept/reject) required" });
    }
    ...
    if (request.status !== "pending") {
      return res
        .status(400)
        .json({ error: "Request already handled", status: request.status });
    }
    ...
    if (io) {
      if (action === "accept") {
        io.to(userRoom(request.requester_id)).emit(
          "live_talk_accepted",
          payload,
        );
...
exports.getTalkStatus = async (req, res) => {
  try {
    ...
    const isHost =
      String(session.user_id?._id || session.user_id) === String(userId);
    const hostDoc = session.user_id;
    const hostRates = resolveUserRates(hostDoc, settings);
    const chatRate = hostRates.chatRatePerMin;
    ...
    if (isHost) {
      ...
      const sessionBeansEarned = activeTalks.reduce(
        (sum, t) => sum + (t.host_beans_earned || 0),
        0,
      );
      hostChatEarnings = {
...
exports.startTalkBilling = async (req, res) => {
  try {
    ...
    if (!sessionId || !requestId) {
      return res
        .status(400)
        .json({ error: "sessionId and requestId required" });
    }
    ...
    if (request.status !== "accepted") {
      return res
        .status(400)
        .json({ error: "Request not accepted yet", status: request.status });
    }
    ...
    if (existing) {
      ...
      return res.json({
        ...
        billingEnabled: settings.liveTalkBillingEnabled,
        canContinue:
          !settings.liveTalkBillingEnabled ||
          (user?.wallet_balance || 0) >= rate,
      });
    }
    ...
    const io = getIo();
    await emitTalkPrivateReady(io, talkSession);
    if (io && chargeResult.diamondsEarned > 0) {
    ...
    res.json({
      ...
      hostDiamondsEarned: chargeResult.diamondsEarned,
      minutesCharged: 1,
      totalCharged: rate,
      ratePerMin: rate,
      billingEnabled: true,
...
exports.tickTalkBilling = async (req, res) => {
  try {
    ...
    if (!sessionId || !talkSessionId) {
      return res
        .status(400)
        .json({ error: "sessionId and talkSessionId required" });
    }
    ...
    const rate =
      talkSession.rate_per_min ||
      (await getHostChatRate(talkSession.host_id, settings));
    ...
    try {
      ...
      if (e instanceof InsufficientWalletError) {
        ...
        await syncHostBusyToLiveRoom(
          talkSession.session_id,
          talkSession.host_id,
        );
        return res.status(400).json({
    ...
    talkSession.host_beans_earned =
      (talkSession.host_beans_earned || 0) + chargeResult.diamondsEarned;
    talkSession.platform_beans_earned =
      (talkSession.platform_beans_earned || 0) +
      (chargeResult.platformBeans || 0);
    ...
    if (io && chargeResult.diamondsEarned > 0) {
      io.to(userRoom(talkSession.host_id)).emit("live_talk_host_earning", {
        ...
        diamondsPerMinute: chargeResult.diamondsEarned,
        talkSessionId: talkSession._id.toString(),
        sessionDiamondsEarned: talkSession.host_beans_earned,
      });
    }
...
exports.endTalkBilling = async (req, res) => {
  try {
    ...
    const hostBeansEarned =
      talkSession && String(talkSession.host_id) === String(userId)
        ? talkSession.host_beans_earned || 0
        : 0;
    ...
    if (talkSession) {
      ...
      if (io) {
        const payload = {
          talkSessionId: talkSession._id.toString(),
          sessionId: String(talkSession.session_id),
          beansEarned: hostBeansEarned,
        };
        io.to(userRoom(talkSession.talker_id)).emit(
          "talk_private_ended",
          payload,
        );
        io.to(userRoom(talkSession.host_id)).emit(
          "talk_private_ended",
          payload,
        );
        io.to(privateTalkRoom(talkSession._id)).emit(
          "talk_private_ended",
          payload,
        );
      }
