...
async function processStickerGift({
  senderId,
  receiverId,
  stickerId,
  sessionId,
  ...
  const sticker = await Sticker.findById(stickerId).select(
    "name price icon_name icon_color bg_color",
  );
...
async function resolveStickerId(stickerId, giftValue) {
  ...
  if (icons?.length) {
    const byIcon = await Sticker.findOne({ icon_name: { $in: icons } }).select(
      "_id",
    );
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("send_gift", async (data) => {
      ...
      try {
        ...
        const isHost2 =
          battle.host2_id && String(hostId) === String(battle.host2_id);
        if (!isHost1 && !isHost2) {
          socket.emit("gift_error", { message: "Invalid gift target" });
          return;
        }
        ...
        const isParticipant =
          String(senderId) === String(battle.host1_id) ||
          (battle.host2_id && String(senderId) === String(battle.host2_id));
        if (isParticipant) {
          socket.emit("gift_error", {
            message: "Only viewers can vote with gifts",
          });
          return;
        }
    ...
    socket.on("pk_opponent_joined", (data) => {
      ...
      io.to(battleId).emit("pk_opponent_joined", {
        battleId,
        user: joinedUser,
      });
    });
    ...
    socket.on("pk_send_sticker", async (data) => {
      const { battleId, userId, username, stickerId, emoji, stickerName } =
        data;
    ...
    socket.on("join_live", async (data) => {
      ...
      try {
        const session =
          await LiveSession.findById(sessionId).select("user_id status");
        if (
    ...
    socket.on("private_talk_send_sticker", async (data) => {
      const { talkSessionId, sessionId, stickerId, emoji, stickerName } =
        data || {};
      const userId = socket.authenticatedUserId || data?.userId;
      ...
      try {
        ...
        if (String(userId) === hostId) {
          socket.emit("private_talk_error", {
            code: "INVALID_GIFT",
            message: "Host cannot send gifts in private chat",
          });
          return;
        }
    ...
    socket.on("live_chat_message", async (data) => {
      ...
      if (trimmed.length > 200) {
        socket.emit("live_chat_error", {
          ...
          message: "Message is too long (max 200 characters)",
        });
        return;
      }
      ...
      try {
        ...
        socket.emit("live_chat_error", {
          ...
          message: "Could not verify chat permission",
        });
        return;
      }
    ...
    socket.on("live_send_sticker", async (data) => {
      ...
      try {
        ...
        if (String(userId) === hostId) {
          socket.emit("live_gift_error", {
            message: "Host cannot send gifts to themselves",
          });
          return;
        }
        ...
        console.log(
          `[Live Sticker] ${username} sent ${stickerName || sticker.name} (${sticker.price} diamonds)`,
        );
    ...
    socket.on("video_call_invite", async (data, ack) => {
      const {
        calleeId,
        callerAvatar,
        channelName,
        callMode: rawCallMode,
      } = data || {};
      ...
      if (!calleeId || !callerId || !channelName) {
        if (typeof ack === "function")
          ack({ success: false, error: "Invalid call invite payload" });
        return;
      }
      ...
      try {
        ...
        const errorPayload = {
          ...
          message:
            permErr.message ||
            "Follow this user and wait until they accept your request.",
        };
        socket.emit("video_call_error", errorPayload);
        if (typeof ack === "function") ack({ success: false, ...errorPayload });
        return;
      }
      ...
      console.log(
        `[VideoCall] ${callerName} → user ${calleeId} channel=${channelName}`,
      );
    ...
    socket.on("video_call_reject", (data) => {
      ...
      if (!callerId) return;
      io.to(userRoom(callerId)).emit("video_call_rejected", {
    ...
    socket.on("video_call_end", (data) => {
      ...
      io.to(userRoom(otherUserId)).emit("video_call_ended", {
        channelName,
        reason,
      });
    });
    ...
    socket.on("disconnect", async () => {
      ...
      if (presenceUserId) {
        const { count, userFullyOffline } = unregisterPresence(
          presenceUserId,
          socket.id,
        );
        emitOnlineCountUpdate(count);
        console.log(
          `User ${presenceUserId} left app presence (${count} active)`,
        );
        ...
        if (userFullyOffline) {
          try {
            ...
            if (!hostingLive) {
              await User.findByIdAndUpdate(presenceUserId, {
                is_online: false,
                status: "offline",
              });
            } else {
